...
const SINGLE_TOOL_QUERY = `*[ _type == "article" && slug.current == $slug ][0] {
  title,
  publishedAt,
  body,
  author,
                ...
                <article className="bg-white rounded-3xl p-10 md:p-16 shadow-[0_20px_50px_rgba(0,0,0,0.2)]">
                    ...
                    {tool.mainImage && (
                        <div className="relative w-full h-64 md:h-96 mb-10 overflow-hidden rounded-2xl">
                            <Image
                                src={urlFor(tool.mainImage).url()}
                                alt={tool.title || 'Article banner'}
                                fill
                                className="object-cover"
                                priority
                            />
                    ...
                    <header className="mb-12 border-b border-slate-200 pb-8">
                        ...
                        <p className="text-slate-500 font-medium">
                            Published on {new Date(tool.publishedAt || Date.now()).toLocaleDateString()}
                        </p>
                    </header>
