...
export default function ProfilePage() {
    ...
    const [userComments, setUserComments] = useState<any[]>([]); // New state for comments
        ...
        <main className="max-w-4xl mx-auto w-full px-4 py-12 min-h-screen">
            <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden mb-8">
                <div className="bg-emerald-900 px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    {!isEditing ? (
                        <button onClick={() => setIsEditing(true)} className="bg-white/10 hover:bg-white/20 text-white px-5 py-2 rounded-full font-bold border border-white/20">Edit Profile</button>
                    ) : (
                        <div className="flex gap-3">
                            <button onClick={() => { setIsEditing(false); setEditFile(null); }} className="text-white font-bold px-4">Cancel</button>
                            <button onClick={handleSaveProfile} disabled={isSaving} className="bg-emerald-500 hover:bg-emerald-400 text-white px-6 py-2 rounded-full font-bold">{isSaving ? 'Saving...' : 'Save'}</button>
                        </div>
                    )}
                </div>
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
                    <div className="md:col-span-2">
                        ...
                        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                            {isEditing ? (
                                <textarea value={editBio} onChange={(e) => setEditBio(e.target.value)} className="w-full p-3 rounded-xl border border-slate-200" rows={4} />
                            ) : (
            ...
            <div className="space-y-6 mt-24">
                <h2 className="text-3xl font-black text-white">Your Recent Comments</h2>
